...
async function main() {
  ...
  // Emit TS data file.
  // RAW_ARTICLES holds the importer-generated lead-paragraph excerpts.
  // articles.excerpts.ts (hand-maintained) overrides them at export
  // time so editorial copy survives re-imports. New article IDs need
  // a matching entry in articles.excerpts.ts — we warn about gaps
  // below.
 ...
 *
 * Excerpts on \`ARTICLES\` are merged from \`articles.excerpts.ts\`
 * (hand-written editorial copy) before export. The raw lead-paragraph
 * excerpts from the importer live in \`RAW_ARTICLES\`. The importer
 * preserves this two-step shape so re-imports do not undo overrides. */
`;
  const body =
    `import { IArticle } from './articles.types';\n` +
    `import { EXCERPT_OVERRIDES } from './articles.excerpts';\n\n` +
    `const RAW_ARTICLES: IArticle[] = ${JSON.stringify(articles, null, 2)};\n\n` +
    `export const ARTICLES: IArticle[] = RAW_ARTICLES.map(a => ({\n` +
    `  ...a,\n` +
    `  excerpt: EXCERPT_OVERRIDES[a.id] ?? a.excerpt,\n` +
    `}));\n`;
  await writeFile(OUT_DATA_FILE, banner + body, 'utf8');

  // Warn on any catalog id missing from articles.excerpts.ts so the
  // editorial team knows which IDs need a hand-written description.
  try {
    const excerptsTs = await readFile(
      resolve(__dirname, '..', 'libs/feature/content/src/lib/articles/articles.excerpts.ts'),
      'utf8',
    );
    const knownIds = new Set();
    const re = /^\s*(\d+)\s*:/gm;
    let m;
    while ((m = re.exec(excerptsTs)) !== null) knownIds.add(parseInt(m[1], 10));
    const missing = articles.map(a => a.id).filter(id => !knownIds.has(id));
    if (missing.length) {
      console.log(`\nNOTE: ${missing.length} article id(s) without hand-written excerpts — add to articles.excerpts.ts: ${missing.join(', ')}`);
    }
  } catch { /* excerpts file may not exist on the first import — that's fine */ }
